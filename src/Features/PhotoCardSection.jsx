...
const PhotoCardSection = () => {
  ...
  return (
    <div className="photo-card-section">
      <h2 className="photo-card-section__title">
        What Do Our Attendees Have To Say?
      </h2>
      <p className="photo-card-section__subtitle">Well, See For Yourself!</p>

      <div className="photo-card-section__controls">
        <button className="photo-card-section__control-button">
          <FaArrowLeft size={24} />
        </button>
        <button className="photo-card-section__control-button">
          <FaArrowRight size={24} />
      ...
      <div className="photo-card-section__cards-container">
        {photoCards.map((card, index) => (
          <div key={index} className="photo-card-section__card">
            <div className="photo-card-section__card-content">
              <div className="photo-card-section__card-image-wrapper">
                <img
                  src={card.image}
                  alt={card.name}
                  className="photo-card-section__card-image"
                />
              </div>
              <h3 className="photo-card-section__card-name">{card.name}</h3>
              <p className="photo-card-section__card-feedback">
                {card.feedback}
              </p>
            </div>
          </div>
